...
class NoiseReducer:
    ...
    def preprocess_data(self, X_train, Y_train):
        dic = {0: 'A', 1: 'B', 2: 'C', 3: 'D', 4: 'E',  5: 'E', 6: 'G', 7: 'H'}
        max_len_trace = max([len(trace) for trace in Y_train])
        for x_trace, y_trace in zip(X_train, Y_train):
            n_additional_columns = max_len_trace - len(x_trace[0])
            if n_additional_columns == 0:
                continue
            # if n_additional_columns > max_len_trace / 2:
            #     TODO: fix - in case step = 1. inserts in the start of the lists
            step = int(max_len_trace / n_additional_columns)
            if step == 1:
                step = 2
                # TODO: consider insert here expansion of the list to max_len_trace
            idxs_to_insert = range(step-1, max_len_trace, step)
            idxs_to_insert = idxs_to_insert[0:min(n_additional_columns, len(y_trace)-1)]
            for idx in idxs_to_insert:
                # y_trace.insert(idx, y_trace[idx - 1]) #'A') # TODO: Fix to 'Z'
                sum_of_vector = 0  # For normalizing the average vector
                for label in range(NUM_OF_LABELS):
                    avg = (x_trace[label][idx - 1] + x_trace[label][idx]) / 2
                    sum_of_vector += avg
                    x_trace[label].insert(idx, avg) # TODO: add normalization (softmax)
                for label in range(NUM_OF_LABELS):
                    x_trace[label][idx] /= sum_of_vector
                y_trace.insert(idx, dic[np.argmax([x_trace[label_idx][idx] for label_idx in range(NUM_OF_LABELS)])])
...
acc_X_before = argmax_of_X(X_train_sandwiches)
...
acc_before = []
for prediction, label in zip(acc_X_before, acc_Y_before):
    acc_before.append(accuracy(np.array(prediction), np.array(label)))

print(acc_before)

X_train_sandwiches, Y_train_sandwiches = model_sandwiches.preprocess_data(X_train_sandwiches, Y_train_sandwiches)
